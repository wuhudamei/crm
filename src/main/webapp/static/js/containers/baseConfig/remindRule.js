            ...
            drawTable: function () {
                ...
                self.$dataTable.on('click', '[data-handle="operate-edit"]', function (e) {
                    ...
                    if(status == '0'){
                    	//需要启用
                    	title = "是否启用【" + name +"】?";
                    	text = "启用[" + name + "],同时将禁用其他提醒规则";
                    }else {
                    	//需要禁用
                    	title = "是否禁用【" + name +"】?";
                    	text = "禁用[" + name + "]后,请手动启用需要的提醒规则,否则系统中将没有提醒规则";
                    }
                    swal({
                        title: title,
                        text: text,
                        ...
                        closeOnConfirm: false
		                    ...
		                    self.$http.get('/api/rule/remindRule/updateStatusById?id=' + id + "&status=" + status).then(function (res) {
		                        if (res.data.code == 1) {
