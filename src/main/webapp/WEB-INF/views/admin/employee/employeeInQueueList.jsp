        ...
        <div class="ibox-content">
            ...
            <div class="panel-group table-responsive">
                <table width="100%" class="table table-striped table-bordered table-hover table-de panel-heading border-bottom-style">
                    ...
                    <tbody>
                    <tr align="center" v-for="(index,item) in employeeInQueueList">
                        <td>{{index + 1}}</td>
                        <td>{{item.storeCode}}</td>
                        <td>{{item.sourceCode}}</td>
                        <td>{{item.empName}}</td>
                        <td>{{item.allowAuto | goType}}</td>
                        <td>{{item.dailyThreshold}}</td>
                        <td>{{item.expectedReward}}</td>
                        <td>{{item.actualReward}}</td>
                    </tr>
                    <tr v-if="employeeInQueueList.length == 0" align="center">
                        <td colspan="8">
                            没有找到匹配的记录
                        </td>
                    </tr>
...
<script src="/static/js/containers/employee/employeeInQueueList.js"></script>