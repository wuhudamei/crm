					...
					<div class="col-md-12 text-left">
						<div class="form-group" id="buttons">
							...
							<a id="createBtn" @click="createBtnClickHandler(!showBtn)" 
								...
								v-cloak :disabled="!showBtn">新增数据字典</a>
							<%-- </shiro:hasPermission> --%>
		...
		<form name="createMirror" novalidate class="form-horizontal" role="form">
			...
			<div class="modal-body">
				...
				<div class="form-group" v-if="dict.parentName != null">
					<label for="parentName" class="col-sm-2 control-label">父类名称:
					</label>
					<div class="col-sm-8">
					...
					<div class="col-sm-8">
						<input v-model="dict.name"
							v-validate:name="{
	                                    ...
	                                    maxlength:{rule:50,message:'字典名称最长不能超过50个字符'}
	                                }"
							maxlength="50" data-tabindex="1" id="name" name="name" type="text"
							class="form-control" placeholder="字典名称"> 
				...
				<div class="form-group"
					...
					<label for="code" class="col-sm-2 control-label">字典编码: </label>
					<div class="col-sm-8">
						<input v-model="dict.code"
							v-validate:code="{
	                                    required:{rule:true,message:'请输入字典编码'},
	                                    maxlength:{rule:20,message:'字典编码最长不能超过20个字符'}
	                                }"
							maxlength="20" data-tabindex="2" id="code" name="code"
							type="text" class="form-control" placeholder="字典编码" :disabled="!isEdit"> 
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input v-model="dict.sort"
							data-tabindex="2" id="sort" name="sort" max="999" 
							type="number" class="form-control" placeholder="排序值"> 
					</div>
				</div>
				...
				<div class="form-group" :class="{'has-error':$validation.remark.invalid && submitBtnClick}">
					...
					<div class="col-sm-8">
						<input v-model="dict.remark"
							v-validate:remark="{
	                                    required:{rule:true,message:'请输入备注'},
	                                    maxlength:{rule:100,message:'备注最长不能超过100个字符'}
	                                }"
							maxlength="100" data-tabindex="2" id="remark" name="remark"
							type="text" class="form-control" placeholder="备注"> 
						<span v-cloak v-if="$validation.remark.invalid && submitBtnClick"
								class="help-absolute"> 
								<span
									v-for="error in $validation.remark.errors"> {{error.message}}
									{{($index !== ($validation.remark.errors.length -1)) ? ',':''}} 
								</span>
						</span>
					</div>
				</div>
...
<script src="${ctx}/static/js/containers/baseConfig/dictlist.js"></script>
