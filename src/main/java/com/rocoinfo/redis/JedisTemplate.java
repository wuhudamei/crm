...
import com.rocoinfo.redis.JedisTemplate.JedisActionNoResult;
...
public class JedisTemplate {
    ...
    /**
     *
     * 函数功能描述:删除所有Key满足匹配规则的项
     * @param keyPattern
     */
    public void delBatch(final String keyPattern){
        execute(new JedisAction<Object>() {
            @Override
            public Object action(Jedis jedis) {
                Set<String> set = getKeys(keyPattern);
	...
	/** 清空list中从0开始到start的数据
	 * @param key
	 * @param start
	 */
	public void cleanListByIndex(final String key, final long start){
		execute(new JedisActionNoResult() {
			@Override
			public void action(Jedis jedis) {
				//ltrim操作，如果start > end 则清空整个List =>等同于 del key
				jedis.ltrim(key, start, -1);
			}
		});
	}
	 ...
	 * @return
	 */
    public String getOneItemByList(final String key) {
        return execute(new JedisAction<String>() {
            @Override
            public String action(Jedis jedis) {
            	List<String> str = jedis.lrange(key, 0, 0);
